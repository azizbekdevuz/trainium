  ...
  if (!cart || cart.items.length === 0) {
    return (
      <div className="mx-auto max-w-5xl px-4 sm:px-6 py-6 sm:py-10">
        ...
        <p className="text-sm sm:text-base text-ui-muted dark:text-ui-faint mt-2">{dict.cart?.empty ?? 'Your cart is empty.'}</p>
        <div className="mt-6">
      ...
      <div className="mt-6 grid gap-6 lg:grid-cols-[1fr_320px]">
        <div className="space-y-4">
          {cart.items.map((it: NonNullable<typeof cart>['items'][0]) => (
            <div key={it.id} className="rounded-2xl border glass-surface p-3 sm:p-4 grid gap-3 sm:gap-4 grid-cols-[80px_1fr] sm:grid-cols-[96px_1fr_auto] items-start">
              <div className="h-20 w-20 sm:h-24 sm:w-24 rounded-xl bg-ui-inset overflow-hidden relative">
                {(() => {
                  const primaryImage = (Array.isArray(it.product.images) && (it.product.images as { src: string }[])[0]?.src) || '';
                  return primaryImage ? (
                    <SmartImage src={primaryImage} alt={it.product.name} fill sizes="96px" className="object-cover" />
                  ) : (
                    <div className="h-full w-full grid place-items-center text-ui-faint text-xs">{dict.cart?.noImage ?? 'No image'}</div>
                  );
                })()}
              </div>
              <div>
                <div className="font-medium">{it.product.name}</div>
                <div className="text-xs text-ui-faint">{it.variant?.name}</div>
        ...
        <aside className="rounded-2xl border glass-surface p-4 sm:p-5 h-max">
          ...
          <div className="mt-4 border-t pt-3 flex items-center justify-between">
            <div className="text-sm text-ui-muted dark:text-ui-faint">{dict.cart?.total ?? 'Total'}</div>
