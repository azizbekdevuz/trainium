...
export function NotificationClient() {
  ...
  const getTypeColor = (type: string) => {
    switch (type) {
      ...
      default:
        return 'bg-ui-inset border-ui-default text-ui-primary';
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="glass-surface rounded-lg border border-[var(--border-default)] p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <select
              ...
              className="w-full border border-ui-default rounded-lg px-3 py-2 focus:ring-2 focus:ring-cyan-500 focus:border-cyan-500"
            >
          ...
          {formData.title && formData.message && (
            <div className="p-4 bg-ui-inset border rounded-lg">
              <h3 className="text-sm font-medium text-ui-secondary mb-2">{dict.admin?.notifications?.form?.preview ?? 'Preview:'}</h3>
      ...
      <div className="glass-surface rounded-lg border border-[var(--border-default)] p-6">
        ...
        <div className="grid gap-3 md:grid-cols-2">
          <button
            ...
            className="p-3 text-left border border-ui-default rounded-lg hover:bg-ui-inset transition"
          >
            <h3 className="font-medium text-sm">{dict.admin?.notifications?.templates?.maintenanceTitle ?? 'System Maintenance'}</h3>
            <p className="text-xs text-ui-muted mt-1">{dict.admin?.notifications?.templates?.maintenanceDesc ?? 'Notify users about scheduled maintenance'}</p>
          </button>
          ...
          >
            ...
            <p className="text-xs text-ui-muted mt-1">{dict.admin?.notifications?.templates?.newProductDesc ?? 'Announce new products to all users'}</p>
          </button>
          ...
          >
            ...
            <p className="text-xs text-ui-muted mt-1">{dict.admin?.notifications?.templates?.holidayDesc ?? 'Inform users about holiday closures'}</p>
          </button>
          ...
          >
            ...
            <p className="text-xs text-ui-muted mt-1">{dict.admin?.notifications?.templates?.flashDesc ?? 'Promote special sales and offers'}</p>
          </button>
        </div>
      </div>
    </div>
